...
public class TurnGroup extends Group {
    private double screenWidth,
                   screenHeight;

    public TurnGroup(double screenWidth, double screenHeight) {
        super();
        this.screenWidth = screenWidth;
        this.screenHeight = screenHeight;
    }

    public void updateRoot(Group gameBoardGroup, String player, int difficulty, int[] directions, Executable mainMenu, Sendable newPlayerTurn) throws Exception {
        ...
        Text playerText = ShapeConstructor.newText(playerString, color,Color.BLACK, screenWidth*0.18,maxTextHeight, screenWidth*0.9,maxTextHeight);

        for(int j = 0 ; j < relics.length ; j++) {
            double centerX = screenWidth*0.8 + maxRelicSize*(j*1.5 + 1);
            try {
                getChildren().add(ShapeConstructor.newImage("relic" + relics[j], maxRelicSize,maxTextHeight*0.8, centerX,maxTextHeight*2));
            } catch (Exception e) {
                getChildren().add(ShapeConstructor.newText(relics[j], Color.CHARTREUSE, maxRelicSize,maxTextHeight*0.8, centerX,maxTextHeight*2));
            }
        }
        ...
        Executable nextTurn = ev -> {
                String[] movements = new String[comboBoxes.length];
                for(int i = 0 ; i < comboBoxes.length ; i++)
                    movements[i] = comboBoxes[i].getValue();
                newPlayerTurn.send(movements);
            };
        Timer timer = new Timer();
        Timeline timeline = new Timeline();
        timeline.setCycleCount(1);
        timeline.getKeyFrames().add(new KeyFrame(Duration.millis(difficulty*1000), new KeyValue(new WritableExecutable(), nextTurn)));
        ...
        getChildren().addAll(playerText, timerText, redDice,redDir, whiteDice,whiteDir, blueDice,blueDir, mainMenuButton,noSave);
