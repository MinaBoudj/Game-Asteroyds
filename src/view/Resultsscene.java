...
//    public Resultsscene(Stage stage, double screenWidth, double screenHeight, String board[][]){  //+ Player l_player[]
    ...
    public class Resultsscene extends Application {
        ...
        Button b_exit = Control_creator.button("Back to main menu", 0.0, 0.0, eventE) ;
        ...
        // TO DO
        String l_player[] = {"J1", "J2", "J3", "J4", "J5", "J6"};
        double length_lp = 3;
        String l_relics[][] = {{"3", "1"}, {"2"}, {"3", "2"}, {"4", "1", "2", "3"}, {"3"}, {"3"}};
        String l_life[] = {"5", "3", "6", "3", "0", "2"};
...
/* 
        ...
        //}
			...
			for (int i = 0 ; i < l_player.length ; i ++ ) {
                container_results.getChildren().add(Control_creator.Label(l_player[i], "#FFF8DC", "cmmi10", 20.0));
                HBox p_info = Control_creator.hbox(5, 10, 0, 0);
                container_results.getChildren().add(p_info);
                for (int j = 0 ; j < l_relics[i].length ; j ++ ) {
                    p_info.getChildren().add(ShapeConstructor.newImage("relic"+l_relics[i][j], screenWidth, screenHeight,  0, 0 ));
                }
                    

                //container_results.getChildren().add(Control_creator.Label("Relics: " + l_relics[i] +  + l_life[i] + "/6 ", "#FFF8DC", "cmmi10", 15.0));
                container_results.getChildren().add(Control_creator.Label(  "Etat du vaisseau : " +l_life[i] , "#FFF8DC", "cmmi10", 15.0));
            }
