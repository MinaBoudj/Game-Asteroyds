...
//    public Resultsscene(Stage stage, double screenWidth, double screenHeight, String board[][]){  //+ Player l_player[]
    public class Resultsscene extends Application {
        ...
        String l_relics[][] = {{"3", "1", "2"}, {"3", "4", "2"}, {"3"}, {"3", "0", "2"}, {"3", "2"}, {"4", "3", "2", "1"}};
        String l_life[] = {"5", "3", "6", "5", "3", "6"};
        //Label p1 = Control_creator.Label(l_player[1], "#FFF8DC", "cmmi10", 20.0);

    
        
    	for (int i = 0 ; i < l_player.length ; i ++ ) {
            HBox p_info = Control_creator.hbox(5.0, 10.0, screenWidth, screenHeight);
            ...
            container_results.getChildren().add(Control_creator.Label(l_player[i], "#FFF8DC", "cmmi10", 20.0));
                ...
                try {
                    container_results.getChildren().add(Control_creator.Label("Relics: " + l_relics[i], "#FFF8DC", "cmmi10", 15.0));
                    p_info.getChildren().add(ShapeConstructor.newImage("relic" + l_relics[i][j], screenWidth, screenHeight, 0, 0.0));
                } catch (Exception e) { }
                container_results.getChildren().add(Control_creator.Label("Etat du vaisseau : "  + l_life[i] + "/6 ", "#FFF8DC", "cmmi10", 15.0));
                }
            } 
